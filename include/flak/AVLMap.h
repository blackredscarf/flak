...
public:
    ...
    typedef AVLMap<Key, T, Compare, PairValueAlloc> Self;

private:
    typedef AVLTree<key_value, value_type, std::_Select1st<value_type>,
...
public:
    ...
    AVLMap(): t_(Compare()) {}
    explicit AVLMap(const Compare& comp): t_(comp) {}

    template <class InputIterator>
    AVLMap(InputIterator first, InputIterator last) : t_(Compare()){
        t_.insertUnique(first, last);
    }

    template <class InputIterator>
    AVLMap(InputIterator first, InputIterator last, const Compare& comp): t_(Compare()){
        t_.insertUnique(first, last);
    }
    ...
    size_type count(const Key& k) { return t_.count(k); }
